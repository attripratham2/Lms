...
export default function Correction() {
  ...
  const hasFinished = assignation?.finished;
      ...
      <Stack justifyContent="center" style={{ overflowY: 'auto' }} ref={scrollRef}>
        <TotalLayoutStepContainer>
          <Stack direction="column" spacing="xl">
            {hasFinished && (
              <Alert severity="success" title={t('finished_alert')} closeable={false}></Alert>
            )}
            <ContextContainer title={t('summary')}>
              <ActivityAccordion>
                {!!instance.metadata.statement && (
                  <ActivityAccordionPanel label={t('instructions')} icon={<InfoIcon />}>
